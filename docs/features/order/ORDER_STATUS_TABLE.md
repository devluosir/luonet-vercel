...
订单状态表表头由 `src/features/order/components/OrderTable.tsx` 渲染，样式与询报价登记表保持一致，保证两个登记类页面的视觉连续性：

- 表头使用浅灰渐变底色，和数据行形成清晰层级
- 列之间使用细分隔线，便于扫描订单号、交货、客户、执行情况和管理员金额列
- 表头文字使用 `text-[11px]`、半粗体、灰色业务文本，不使用大写字距样式
- 可排序列（订单编号、交货）使用按钮样式；当前排序列以蓝色文字、浅底和细 ring 高亮
- 非排序列保持纯文本表头，避免误导用户点击
- 小屏继续使用短标签，例如订单编号显示为「编号」、执行情况显示为「执行」

本次表头样式调整已在 2026-06-26 手动验证完成；未改变筛选、排序、行内编辑、PDF/Excel 或 D1 schema。
...
|------|-----------------|------|------|
| 无 | `undefined`（兼容历史 `null`） | 粗体订单编号文本 | 「正常」 |
...
C/P/S 的情况备注保存于询价记录 `orderSubStatusRemark`，由编辑询价弹窗维护。订单状态表会把该备注显示在「客户订单号」下方，并按状态保留颜色：辙销红色、悬挂绿色、善后蓝色；客户详情活动列表也会同步展示该备注。
...
- 「进行中」= 非辙销订单中，执行情况为空、以 `备货` 开头、以 `交货` 开头，或订单标记为悬挂P/善后S；点击后默认切到全部时间 + 订单号降序排序
- 「正常」= `orderSubStatus` 未设置（`undefined`，兼容历史 `null`）或为 'suspended'（含悬挂）；订单状态表与采购订单表共用 `isNormalOrder` 判定，列表与角标口径一致
...
颜色应用到订单号、交货日期、客户、内容简述、确认日、客户订单号、执行情况，以及管理员可见的金额、回款、到账金额。订单号下方的 `inquiryNo` 仍固定为灰色；客户订单号下方的 C/P/S 备注使用独立状态色；空值占位（如 `m.D`、`执行情况`、`¥/$`、`—`）继续使用淡灰色，避免误认为已有数据。
...
- [x] 筛选面板：时间范围 + 订单状态芯片 + 进行中 + 搜索 + 客户选择器
- [x] 表头样式：与询报价登记表统一的浅灰层级、列分隔和排序高亮
- [x] 同步时间插槽（`topBarSlot`）

### Phase 5：验证

- [x] 2026-06-26 手动验证表头样式、搜索/客户筛选、订单号显示、行颜色规则
- [x] TypeScript / ESLint 针对性检查通过
- [ ] 修改订单追踪字段后，D1 同步正常
