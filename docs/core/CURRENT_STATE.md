...
| `/quotation` | 外贸报价合同（报价单 / 销售确认） | 已合并为同一入口；外贸报价顶部可单向“转为外贸合同”，原地切换且保留表单数据；本地历史为主，支持 PDF/Excel、复制、编辑 |
| `/quotation?tab=domestic` | 内销报价合同 | 独立侧边栏入口；内销报价顶部可单向“转为内销合同”，复用既有条款替换与编辑确认保护，设置面板继续保留双向类型切换；历史记录使用独立 `type='domestic'` |
...
- 客户页支持列表/卡片视图、分类筛选、搜索、详情页，以及独立的“统计分析”视图；统计视图展示全部客户询价/已报价/订单趋势、可按询价数/已报价数/订单数切换依据的 Top 10 横向排名，以及 A/B/C/New/黑名单分类占比，进入时隐藏资料列表相关操作。
