...
- “编辑订单”弹窗不再展示“客户询价编号”；只读区保留订单编号、询价编号、联络人和内容简述。编辑区桌面将客户订单号/金额按 75% / 25% 排布，将执行情况/回款月份/到账金额按 50% / 25% / 25% 排布，小屏统一纵向堆叠；无财务权限时只渲染客户订单号和执行情况，并保留各自的 3/4、1/2 桌面跨度（TASK-180）。
