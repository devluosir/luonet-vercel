...
- 移动端底部导航（`src/components/layout/MobileBottomTab.tsx`，`md:hidden`）改为固定 5 个分类入口：新建 / 登记 / 管理 / 工具 / 我，点击后弹出浮动子菜单而非直接跳页。「新建」子项复用 `dashboardModules.ts` 的 `QUICK_CREATE_MODULES`；「登记/管理/工具」子项与桌面端 `AppSidebar.tsx` 的 `NAV_ITEMS` 保持同一套 id 和权限 moduleId（工具分类不含 AI 邮件）；权限过滤后子项全空则对应顶层入口隐藏，「我」入口固定常驻。「我」菜单含关于（占位弹窗，内容待补充）、个人信息（见下）、退出登录。
