...
- 详情页“打开采购部登记”携带供应商 ID 与名称快照；落地页自动切换到全部时间并按主档 ID 精确筛选，供应商改名后仍可命中旧记录，手动下拉继续按名称筛选。
...
- 外贸报价单顶部操作区新增“转为外贸合同”图文按钮，复用现有 tab 切换将当前表单原地转换为销售确认；URL 不跳转，客户、货品和金额数据保持，标题、面包屑与历史类型随 store 自动更新。
- 内销报价单顶部新增同款“转为内销合同”按钮，复用既有内销类型切换逻辑；默认条款随合同模板更新，手工编辑过条款时继续使用原有二次确认保护。合同状态下快捷按钮隐藏，设置面板原双向切换入口保留。
- 顶部标题与操作区在小屏改为上下分行并允许操作按钮换行，避免新增文字按钮挤压标题或产生横向溢出。

#### 客户详情与客户管理统计分析（TASK-182/183/184）
- 客户详情资料卡下新增当前客户专属的询价/已报价/订单趋势图，复用首页统计口径并支持天、周、月、季、年粒度；仅按 `customerId` 汇总非删除记录，供应商和收货人详情保持不变。
- 客户管理标签栏新增独立“统计分析”视图，展示全部客户趋势、询价 Top 10 横向排名和 A/B/C/New/黑名单分类环形图；进入统计时隐藏新增、搜索、筛选和资料列表，点击任一业务标签即可退出。
- 客户排名卡片支持在询价数、已报价数和订单数三种 Top 10 排序依据间切换，标题随指标更新，三组条形指标与图例始终保留；默认询价排序兼容原行为。
- 统计数据继续由客户端询价 store 派生，不新增后端接口；入口只受客户管理权限控制，不额外依赖询报价模块权限。

#### 管理后台用户详情弹窗紧凑布局（TASK-181）
- 管理员和账户状态开关移至用户信息头右侧，保留当前用户管理员开关禁用提示及保存/删除期间的禁用行为；移除独立“账户设置”卡片和管理员业务权限提示条。
- “登记表”权限在桌面改为询报价/订单状态与采购部登记/采购订单表左右两列，各自高级权限紧跟父模块；小屏退化为单列。其它权限分类、重置、保存和删除逻辑保持不变。
- 追加当前用户自锁保护：当前登录用户不能停用自己的账户，也不能取消自己的管理员身份；界面禁用对应开关，Next 管理 API 同步拒绝绕过前端的自我停用/降权请求。其他管理员管理目标账号的能力不受影响。

#### 订单编辑弹窗信息密度优化（TASK-180）
- “编辑订单”弹窗移除冗余的“客户询价编号”，只读区保留订单编号、询价编号、联络人和内容简述两行两列。
- 有财务权限时，桌面将客户订单号/金额按 75% / 25% 排布，执行情况/回款月份/到账金额按 50% / 25% / 25% 排布；小屏将客户订单号和执行情况各自整行展示，再将金额/回款月份/到账金额三等分同排。无财务权限时保留客户订单号和执行情况原定跨度，不渲染三个财务字段。
- 小屏只读区将订单编号与联络人同排、询价编号与内容简述同排，交货日期与确认日期也保持同排；日期、月份和金额输入增加最小宽度收缩保护，避免窄屏溢出。
- 订单状态标记、执行情况预设与收货人选择、财务权限判断及保存逻辑保持不变。

#### 采购订单编辑弹窗信息密度优化（TASK-179）
- “编辑采购订单”弹窗移除冗余的“客户询价编号”；只读信息改为两组三列，并将订单状态标记保留为独立整行。
- 可编辑区桌面按“交货日期 25% / 采购金额 25% / 执行情况 50%”排布；小屏将交货日期与采购金额并排，执行情况独占下一行。无财务权限时桌面按约 1/3 / 2/3 排布，小屏纵向堆叠。
- 仅调整展示结构和响应式栅格，字段来源、权限判断、预设按钮、收货人选择及保存补丁逻辑保持不变。

#### 三张登记表统一整行弹窗编辑（TASK-174/175/176）
- 订单状态表取消交货/确认日期、客户订单号、执行情况、金额、回款月份和到账金额的全部行内编辑；所有业务单元格点击统一打开“编辑订单”弹窗，批量选择 checkbox 继续独立工作，不触发弹窗。
- 采购部登记“内容描述”取消行内输入，点击该格与其它格一致打开“编辑询价”弹窗；采购订单表的采购单号、供应商、金额、交货日期、执行情况等列统一改为整行点击打开“编辑采购订单”弹窗。
- 三张表保留原有只读格式、C/P/S 与 S-OK 展示、金额权限和响应式列显隐；删除无调用方的 `DeliveryStatusCell` 及三套瞬时行内编辑状态，弹窗共用的执行情况预设移至独立常量文件。

#### 询报价登记移除操作列（TASK-169）
- 列表删除“操作”表头和行内软删除按钮，点击资料行继续打开编辑弹窗；批量选择工具栏的删除保持原有软删除语义和 `inquiry.batchEdit` 权限门。

#### 采购供应商详情字段与双栏布局（TASK-167/168/170）
- 采购活动移除销售侧“客户询价编号”，改为显示采购部登记共享的“内容描述”；空描述显示占位符，其余状态、订单和时间信息不变。
- 详情信息卡的“基本信息”和“采购设置”在桌面宽度下改为带竖向分隔的左右双栏，移动端仍上下堆叠；“基本信息”保持每项独占一行，“采购设置”将供应范围/类型、付款条件/币种分别两两并排，备注独占整行，联系人继续独占下方整行。

#### 采购供应商列表名称堆叠（TASK-166）
- 列表改为“供应商 / 主联系人 / 供货范围”三列业务信息；供应商全称作为加粗主行，简称在下方以小字显示，行高同步收紧。

#### 采购供应商详情页紧凑布局（TASK-165）
- “基本信息”和“采购设置”收紧分组、标题与字段的垂直间距，输入框及保存/取消按钮尺寸保持不变。
- 采购活动在桌面宽度下改为单行展示询价编号、状态、已转订单、内容描述、报价日期、订单号和时间；长内容截断并保留 title，移动端继续允许自然换行。
- 仅调整布局样式，供应商 ID 精确匹配、活动排序、空状态、字段保存及权限逻辑保持不变。
...
- **追加修复**：销售侧已关闭/已回复客户无法报价时，采购部登记表整行（询价编号+内容描述）文字颜色现也会变灰，不再只是状态列 badge 变化。新增 `getPurchaseRowColorClass`，复用 `computePurchaseMainStatus` 的最高两档优先级判断，与状态列共用同一份口径。
- **追加修复**：编辑询价弹窗"询报价状态"区域"已报价"行尾部的"其他 n 家已报价"只读提示补上日期（此前只有状态列同名 badge 带日期），复用与状态列相同的 `findLatestOtherQuotedDate`。
...
- 受限视图（仅采购权限）新增只读暴露该字段，保证采购订单表也能正确显示"S-OK"。
...
- **追加修复**：四张表初版实现在拖拽断点下都不撑满窗口（右侧留白）。改为每张表挑一列内容型列（内容描述/内容简述）不设显式宽度、不给手柄，交给 `table-layout: fixed` 把 `w-full` 减去其它列显式宽度后的剩余空间全部分给它，表格始终撑满容器。
- **追加调整**：采购部登记表"状态"列改名为"状态描述"，各状态文案带上最贴切来源的日期（已关闭/已成单/已补充信息/需补充信息/其他 n 家已报价），多来源取较新日期，无日期时不带空括号。过程中修复一处真实回归：need_info 供应商没填日期时曾被误判成"不存在"、状态列错误跳到更低优先级，已修复并补了回归测试；"已补充信息"优先级高于"需补充信息"的判断本来就正确，逐条核实后确认无需改动。
- **追加修复（调宽方向反了）**：上一条"追加修复"里挑出来不设显式宽度、吸收剩余空间的撑满列（内容描述/内容简述）被放在了每张表偏靠前的位置（第 2 列），但后面还有多个可拖拽列——拖动这些列的手柄时，多出/减少的宽度要靠撑满列补偿，而撑满列在左边，视觉上就变成"往左扩展"而不是正常的"往右扩展"；撑满列本身又没有手柄，用户还会感知成"手柄选不中"。修复：把撑满列的位置挪到每张表渲染顺序里**实际最后一列**（`PurchaseRegistrationTable` 的"状态描述"、`InquiryTable` 的"询报价状态"、`PurchaseOrderTable` 的"执行情况"、`OrderTable` 按 `canViewFinancials` 权限动态取"到账金额"或"执行情况"），"内容描述/内容简述"改回正常可拖拽列。
- **追加修复（末列被压缩导致表头增高）**：撑满列原先没有最小宽度保护，当前面各列的默认/已保存像素宽度接近或超过容器宽度时，最后一列会被压到 0～几十像素；采购订单表裸文本“执行情况”因此逐字换行并抬高整行，其余两表虽有截断但会隐藏末列。新增可测试的 `computeResizableTableMinWidth`，按当前列宽为末列预留最小空间，容器不足时由既有横向滚动承接；询报价、订单状态、采购订单三表的普通表头标签同时统一为固定 `h-6`、不换行、溢出截断，表头高度不再受列宽影响。
- **追加修复（已补充信息被影子记录覆盖）**：`PurchaseRegistrationPage.tsx` 为了让筛选栏"报价状态"按采购部自己的 `purchaseQuotedStatuses` 筛选，构造了一份把 `quotedStatuses` 替换成 `purchaseQuotedStatuses` 的"影子记录"数组喂给筛选/排序 hook，但筛选排序完成后没有换回真实记录就直接传给了表格——表格状态列因此读到错误的 `quotedStatuses`，销售侧真实的"已补充信息"记录被覆盖看不到，`computePurchaseMainStatus` 误判成更低优先级的"需补充信息"（编辑弹窗按 id 直接从 store 查最新记录，不经过这层影子记录，显示是正确的，两处不一致由此暴露）。新增纯函数 `restoreOriginalRecords`，筛选排序完成后按 id 把最终渲染用的记录换回原始对象。顺带把编辑询价弹窗头部的询价编号从灰色小字改为醒目的蓝色粗体。
