...
const getEventDate = (item: TimelineHistoryRecord): string => (
  item.data?.date || item.createdAt || new Date().toISOString().split('T')[0]
);
...
export function extractTimelineFromQuotationHistory() {
  try {
    ...
    quotationHistory.forEach((item: TimelineHistoryRecord) => {
...
export function extractTimelineFromPackingHistory() {
  try {
    ...
    packingHistory.forEach((item: TimelineHistoryRecord) => {
...
export function extractTimelineFromInvoiceHistory() {
  try {
    ...
    invoiceHistory.forEach((item: TimelineHistoryRecord) => {
      if (!item.data || !item.customerName) return;
