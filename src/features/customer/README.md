...
- 客户/供应商/收货人详情页的名称和地址支持行内编辑，不再打开整表单编辑弹窗。
- 收货人管理的列表/卡片视图显示收货订单数量，不显示主联络人摘要。
- 客户详情页显示资料、联络人、统计、活动列表和跟进；收货人详情页隐藏联络人区块，显示「收货订单」。
- 收货人详情页的「收货订单」按订单记录 `orderDeliveryConsignee` 精确匹配；该字段由订单状态表维护，状态从「交货」改为「发票」或「备货」时不应被自动清空。
- 活动列表按 `customerId`、`contactId`、旧 `inquirer` 文本兜底匹配该客户全部联络人的询价。
- 活动列表左侧编号优先显示订单编号；无订单编号时显示询价编号。
- 活动列表会区分询价订单附加状态：`cancelled` 显示「已辙销」、`suspended` 显示「已悬挂」、`followup` 显示「善后」；存在对应订单时显示状态筛选按钮，`orderSubStatusRemark` 会追加到活动描述中并按状态着色（红/绿/蓝）。
