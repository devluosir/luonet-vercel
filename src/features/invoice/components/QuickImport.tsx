...
export function QuickImport({
  ...
  useEffect(() => {
    ...
    const handlePaste = (e: ClipboardEvent) => {
      ...
      if (text.trim()) {
        // 自动处理粘贴的数据
        const parsedItems = parseInvoiceData(text);
        if (parsedItems.length > 0) {
          onInsert(parsedItems, true); // 替换模式
          closeAll();
        }
      }
    };
      ...
      {open && (
        <div className="fixed inset-0 z-50" onClick={closeAll}>
          ...
          >
            ...
            >
              <h3 className="text-sm font-medium text-[#1D1D1F] dark:text-[#F5F5F7]">快速导入发票数据</h3>
              <button
                type="button"
                onClick={closeAll}
            ...
            <div className="p-6">
              <div className="text-center space-y-4">
                ...
                <div className="text-sm text-[#86868B] dark:text-[#86868B] space-y-2">
                  <p>支持从Excel发票表格复制数据</p>
                  <p>将发票表格的内容复制，直接粘贴即可</p>
                  <p>包含表头、序号、备注的区域</p>
                  <p>无序号的行不用复制进来</p>
                </div>
                <div
