...
export default function PurchaseHistoryTab({ 
  ...
  return (
    <div className="overflow-hidden">
      ...
      <div className={`bg-gradient-to-r px-4 py-2 sm:py-4 border-b border-gray-200 dark:border-gray-600 ${
        ...
        <div className="flex items-center w-full">
          ...
          <button
            ...
            className="flex-1 min-w-0 truncate font-semibold pl-2 text-left hover:text-orange-600 dark:hover:text-orange-400 transition-colors flex items-center group whitespace-nowrap text-gray-900 dark:text-white"
          >
            供应商名称
          ...
          <button
            ...
            className="w-24 sm:w-40 flex-shrink-0 font-semibold text-gray-900 dark:text-white text-left hover:text-orange-600 dark:hover:text-orange-400 transition-colors flex items-center group whitespace-nowrap"
          >
            采购单号
          ...
          <button
            ...
            className="hidden md:flex w-36 flex-shrink-0 font-semibold text-gray-900 dark:text-white text-left hover:text-orange-600 dark:hover:text-orange-400 transition-colors items-center group whitespace-nowrap"
          >
            金额
          ...
          <button
            ...
            className="hidden lg:flex w-40 flex-shrink-0 font-semibold text-gray-900 dark:text-white text-left hover:text-orange-600 dark:hover:text-orange-400 transition-colors items-center group whitespace-nowrap"
          >
            修改时间
            ...
            >
              <div className="flex items-center w-full">
                ...
                <div className="flex-1 min-w-0 truncate text-xs sm:text-sm font-medium text-gray-900 dark:text-white pl-2" title={item.supplierName}>
                  {item.supplierName ? item.supplierName.split('\n')[0]?.trim() || item.supplierName : '未命名供应商'}
                </div>
      ...
      {/* 金额统计 */}
      {history.length > 0 && (
        <div className="bg-gradient-to-r from-orange-50 to-red-50 dark:from-orange-900/20 dark:to-red-900/20 border-t border-orange-200 dark:border-orange-800 px-4 py-2 sm:py-3">
          <div className="text-right text-xs sm:text-sm text-gray-700 dark:text-gray-300 font-medium">
            {Object.entries(history.reduce((acc, item) => {
              ...
              <span key={currency} className="mr-4">
                {currency} 合计：
                <span className="font-bold text-orange-600 dark:text-orange-400">{total.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </span>
            ))}
          </div>
        </div>
